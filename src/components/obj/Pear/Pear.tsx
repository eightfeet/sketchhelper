...
const Pear: React.FC<Props> = ({ visible, opacity, showEdige, shadow, ...groups }) => {
    const { nodes } = useGLTF(`${process.env.PUBLIC_URL || ''}/glb/pear.glb`) as any;
    

    return (
        <group {...groups} >
            {/* <Center position={[0, 1.15, 0]}> */}
            <group position={[0, -0.98, 0]} rotation={[Math.PI / 180 * 90, 0, 0]} scale={0.025}>
                <mesh
                    scale={2}
