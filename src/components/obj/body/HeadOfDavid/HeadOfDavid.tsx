...
const HeadOfDavid: React.FC<Props> = ({ visible, opacity, showEdige, shadow, ...groups }) => {
    const { nodes } = useGLTF(`${process.env.PUBLIC_URL || ''}/glb/body/head_of_david.glb`) as any;

    return (
        <group {...groups} >
            <Center position={[0,1.01,0.4]} rotation={[0,angleToRotation(0),0]}>
                <group position={[0, 8, 0]} rotation={[angleToRotation(180), angleToRotation(-90), 0]} scale={0.01}>
                    <mesh
                        scale={2}
